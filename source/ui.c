...
static inline int get_shake_x(void) {
    int shake = 0;
    if (g_major_shake > 0) {
        shake += (rand() % 25) - 12;
    } else {
...
static inline int get_shake_y(void) {
    int shake = 0;
    if (g_major_shake > 0) {
        shake += (rand() % 25) - 12;
    } else {
...
static void clearScreenPsychedelic(u32 *b, u32 st) {
    s_frame_count++;
    if (g_major_shake > 0) g_major_shake--;
    u32 sw = st / sizeof(u32);
    int offset = (int)(s_frame_count * 5);
    for (int y = 0; y < FB_H; y++) {
        int y_val = (y + offset) & 0xFF;
        for (int x = 0; x < FB_W; x++) {
            int x_val = (x - offset) & 0xFF;
            b[y * sw + x] = RGBA8(x_val, y_val, x_val ^ y_val, 255);
        }
    }
}
...
static void blitImg(u32 *b, u32 st, int dx, int dy, int dw, int dh, u8 *img) {
    ...
    float scale = 1.0f + 0.15f * sinf((float)s_frame_count * 0.12f);
    int new_dw = (int)(dw * scale);
    int new_dh = (int)(dh * scale);
    dx = dx - (new_dw - dw) / 2;
    dy = dy - (new_dh - dh) / 2;
    dw = new_dw;
    dh = new_dh;
    ...
    for (int y = 0; y < dh; y++) {
        ...
        for (int x = 0; x < dw; x++) {
            ...
            putPixel(b, st, dx + x, dy + y, blendPix(d, r, g, bb, a));
        }
    }
}
...
static void drawS2Bar(u32 *b, u32 st, bool focused) {
    ...
    int sy = S2BAR_Y + shY;
    if (focused)
        roundedCard(b, st, sx - 4, sy - 4, S2BAR_W + 8, S2BAR_H + 8, 20, packColor(C_S2));
    roundedCard(b, st, sx, sy, S2BAR_W, S2BAR_H, 16,
                packColor(focused ? C_CARD_SEL : C_CARD));
    drawCF(b, st, s_semi, FB_W / 2 + shX, sy + 40, 25,
...
void ui_draw_picker(int selection, int current, int focus, const char *status, int updVer) {
    ...
    if (updVer > 0) {
        ...
        drawCF(b, st, s_semi, FB_W / 2 + shX, 32 + shY, 20, packColor(C_RED), m);
    }

    drawCF(b, st, s_bold, FB_W / 2 + shX, 84 + shY, 42, packColor(C_TITLE), lang_str(STR_TITLE_PRELUDE));
    // sous-titre centre : "Mode actuel : NEXTENDO" (le nom du mode en sa couleur)
    {
        ...
        drawF(b, st, s_reg,  sx + shX,      120 + shY, 23, packColor(C_SUBTLE), lbl);
        drawF(b, st, s_semi, sx + wl + shX, 120 + shY, 23,
...
void ui_draw_confirm(int selection, bool warnNoEmummc) {
    ...
    int cw = 820, ch = warn ? 500 : 380, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
...
void ui_draw_s2_info(void) {
    ...
    int cw = 940, ch = 440, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
    roundedCard(b, st, cxx - 4, cyy - 4, cw + 8, ch + 8, 28, acc);
    roundedCard(b, st, cxx, cyy, cw, ch, 24, packColor(C_CARD));
...
void ui_draw_progress(const char *line) {
    ...
    drawCF(b, st, s_bold, FB_W / 2 + shX, FB_H / 2 - 24 + shY, 36, packColor(C_TITLE), lang_str(STR_PROGRESS_TITLE));
    drawCF(b, st, s_semi, FB_W / 2 + shX, FB_H / 2 + 30 + shY, 26, packColor(C_S2), line ? line : lang_str(STR_PROGRESS_WAIT));
...
void ui_draw_result(const char *title, const char *msg, bool ok) {
    ...
    int cw = 940, ch = 300, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
    roundedCard(b, st, cxx - 4, cyy - 4, cw + 8, ch + 8, 28, acc);
    roundedCard(b, st, cxx, cyy, cw, ch, 24, packColor(C_CARD));
...
void ui_draw_lang_menu(int sel) {
    ...
    int cw = 700, ch = 460, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
    roundedCard(b, st, cxx - 4, cyy - 4, cw + 8, ch + 8, 28, acc);
    roundedCard(b, st, cxx, cyy, cw, ch, 24, packColor(C_CARD));
    ...
    static const StringID lang_ids[3] = { STR_LANG_EN, STR_LANG_ES, STR_LANG_PT };
    for (int i = 0; i < 3; i++) {
        int rowY = cyy + 120 + i * 80;
        bool hovered = (i == sel);
...
void ui_draw_upd_confirm(int buildVer) {
    ...
    u32 acc = packColor(C_BLUE);
    int cw = 700, ch = 360, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
    roundedCard(b, st, cxx - 4, cyy - 4, cw + 8, ch + 8, 28, acc);
    roundedCard(b, st, cxx, cyy, cw, ch, 24, packColor(C_CARD));
...
void ui_draw_nintendo_coming(int distance, bool inMeters) {
    ...
    u32 acc = packColor(C_WARN);
    int cw = 960, ch = 480, cxx = (FB_W - cw) / 2 + shX, cyy = (FB_H - ch) / 2 + shY;
    roundedCard(b, st, cxx - 4, cyy - 4, cw + 8, ch + 8, 28, acc);
    roundedCard(b, st, cxx, cyy, cw, ch, 24, packColor(C_CARD));

    int cx = FB_W / 2 + shX;
    drawCF(b, st, s_bold, cx, cyy + 80, 44, packColor(C_RED), "NINTENDO IS COMING !!!");
    drawCF(b, st, s_semi, cx, cyy + 160, 28, packColor(C_TITLE), "THE LAWYERS HAVE BEEN DISPATCHED.");
    drawCF(b, st, s_reg, cx, cyy + 220, 24, packColor(C_SUBTLE), "RUN. HIDE. IT IS TOO LATE.");
    
    char distBuf[128];
    if (inMeters) {
        snprintf(distBuf, sizeof(distBuf), "DISTANCE: %d m", distance);
    } else {
        snprintf(distBuf, sizeof(distBuf), "DISTANCE: %d km", distance);
    }
    
    u32 distCol = (s_frame_count % 10 < 5) ? packColor(C_TITLE) : packColor(C_RED);
    drawCF(b, st, s_bold, cx, cyy + 340, 54, distCol, distBuf);
...
void ui_draw_fake_ban(void) {
    ...
    for (int y = 0; y < FB_H; y++)
        for (int x = 0; x < FB_W; x++) b[y * sw + x] = RGBA8(0, 0, 0, 255);

    int cx = FB_W / 2;
    drawCF(b, st, s_bold, cx, 180, 44, RGBA8(255, 255, 255, 255), "Error Code: 666-666");
    
    drawCF(b, st, s_semi, cx, 300, 28, RGBA8(255, 255, 255, 255), "This console has been permanently banned");
    drawCF(b, st, s_semi, cx, 340, 28, RGBA8(255, 255, 255, 255), "from Nintendo Network online services");
    drawCF(b, st, s_semi, cx, 380, 28, RGBA8(255, 255, 255, 255), "due to unauthorized WTF modifications.");
    
    drawCF(b, st, s_reg, cx, 520, 22, RGBA8(180, 180, 180, 255), "Press the POWER button to shutdown.");
    drawCF(b, st, s_reg, cx, 560, 22, RGBA8(180, 180, 180, 255), "Press any button to exit.");
